...
from functions.copilot.context import COPILOT_CONTEXT
...
def process_copilot_response(response: str):
    """
    ...
    try:
        ...
        st.session_state['resposta_copilot'] = response
    except Exception as error:
        st.error(f"Error processing oracle response: {error}")

def handle_copilot_flow(chat_interaction, chain, memory):
    """
    Handle the copilot-specific chat flow.
    ...
    chat_message = 'Como posso te ajudar hoje?' if current_lang == 'pt_BR' else 'How can I help you today?'
    response = chat_interaction(chat_message, chain, memory, key="copilot_chat")
    
    if response:
        process_copilot_response(response)
        st.rerun()
