...
class GeoCommerceAnalyzer:
    ...
    def __init__(self):
        self.config = None # Removed GeoCommerceConfig import, so self.config is no longer available
    ...
    def filter_data(self, df: pd.DataFrame, filters: Dict[str, Any]) -> pd.DataFrame:
        ...
        if 'created_at' in filtered_df.columns:
            ...
            if date_range and date_range != 'All time':
                date_ranges = self.config.get_date_range_options() if self.config else {} # Assuming self.config is available if not removed
    ...
    def create_geographic_distribution_map(self, df: pd.DataFrame, 
        ...
        if map_data.empty:
            ...
            m = folium.Map(
                location=self.config.DEFAULT_MAP_CENTER if self.config else [0, 0], # Assuming self.config is available if not removed
                zoom_start=self.config.DEFAULT_MAP_ZOOM if self.config else 10, # Assuming self.config is available if not removed
                tiles='OpenStreetMap'
            )
            return m
    ...
    def create_province_analysis(self, df: pd.DataFrame) -> Tuple[go.Figure, go.Figure]:
        ...
        fig1.update_layout(height=self.config.CHART_HEIGHT if self.config else 400) # Assuming self.config is available if not removed
        ...
        fig2.update_layout(height=self.config.CHART_HEIGHT if self.config else 400) # Assuming self.config is available if not removed
    ...
    def create_temporal_analysis(self, df: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(
            ...
            height=self.config.CHART_HEIGHT if self.config else 400, # Assuming self.config is available if not removed
            hovermode='x unified'
        )
    ...
    def export_data(self, df: pd.DataFrame, format_type: str = 'CSV') -> bytes:
        ...
        # Limit export size
        if len(df) > self.config.MAX_EXPORT_ROWS if self.config else 10000: # Assuming self.config is available if not removed
            df = df.head(self.config.MAX_EXPORT_ROWS if self.config else 10000) # Assuming self.config is available if not removed
