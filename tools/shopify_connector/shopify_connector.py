...
class ShopifyGraphQLClient:
    ...
    async def fetch_sales_data_paginated(self, limit: int = 50, cursor: Optional[str] = None,
                                       created_since: Optional[datetime] = None,
                                       product_filter: Optional[str] = None) -> Dict:
        """Fetch sales data focused on revenue calculations that match Shopify admin"""
        ...
        # Product filter for specific product analysis
        if product_filter:
            # Use a contains search to match Shopify admin behavior
            filter_parts.append(f'product_title:*{product_filter}*')
    ...
    async def fetch_all_sales_data(self, created_since: Optional[datetime] = None,
                                 progress_callback=None, product_filter: Optional[str] = None) -> List[Dict]:
        """Fetch all sales data with pagination, focused on accurate revenue calculation"""
        all_sales = []
        cursor = None
        page_count = 0
        ...
        while True:
            try:
                result = await self.fetch_sales_data_paginated(
                    limit=50, 
                    cursor=cursor, 
                ...
                for edge in edges:
                    order = edge.get('node', {})
                    all_sales.append(order)
                
                page_count += 1
                
                if progress_callback:
                    progress_callback(len(all_sales), page_count)
                ...
                print(f"Error fetching sales data page {page_count}: {e}")
                break
