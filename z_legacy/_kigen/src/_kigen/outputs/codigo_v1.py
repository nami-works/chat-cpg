...
def classificar_despesa(despesa: str) -> (str, str):
    ...
    resultado = openai.Completion.create(
        ...
        prompt=f"Classifique a despesa: {despesa}",
        max_tokens=60
    )
    resposta = resultado['choices'][0]['text'].strip().split('|')
    return resposta[0], resposta[1] if len(resposta) > 1 else "Sem justificativa"
...
def interpretar_comando(comando: str) -> str:
    ...
    resultado = openai.Completion.create(
        engine="text-davinci-002",
        prompt=f"Interprete o comando: {comando}",
        max_tokens=60
    )
...
def main():
    try:
        ...
        print(f"❌ Erro ao calcular a receita líquida: {e}")
    ...
    try:
        ...
        print(f"❌ Erro ao calcular CMV: {e}")
        cmv = None
    ...
    try:
        ...
        print(f"❌ Erro ao calcular margem bruta: {e}")
        margem_bruta = None
    ...
    try:
        ...
        print(f"❌ Erro ao calcular margem de contribuição: {e}")
    ...
    try:
       .    despesa = "Compra de papel"
        ...
        print(f"❌ Erro ao classificar despesa: {e}")
